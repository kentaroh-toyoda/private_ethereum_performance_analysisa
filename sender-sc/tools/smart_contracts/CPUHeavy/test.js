...
yargs.command('$0', 'Test CPUHeavy', {
	...
	'length': {
		alias: 'l',
		default: 100,
		describe: 'Number of elements to be sorted'
	},
	'signature': {
		default: 0,
		describe: 'Signature'
	...
	let sort = (nonce) => {
		return new Promise(async resolve => {
			const txData = contract.methods.sort(parseInt(argv['length']), parseInt(argv['signature'])).encodeABI()
			const txParams = {
				from: fromAddr,
	...
	await pMap(nonces, sort, { concurrency: Infinity })

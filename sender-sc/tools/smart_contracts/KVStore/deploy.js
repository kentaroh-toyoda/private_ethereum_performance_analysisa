...
yargs.command('$0', 'Deploy KVStore', {
  'from': {
    alias: 's',
    default: 0,
  ...
  // 1. compile
  const inputs = {
    'kvstore.sol': fs.readFileSync('./KVStore/kvstore.sol', 'utf8')
  }
  ...
  // 2. deploy
  const abi = compiledContract.contracts['kvstore.sol:KVStore'].interface
  const bytecode = '0x' + compiledContract.contracts['kvstore.sol:KVStore'].bytecode
